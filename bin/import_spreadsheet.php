...
function normalizeVendor(string $rawVendor, string $description, DateTimeImmutable $reference, Worksheet $worksheet, int $row, array $base): array
{
    ...
    $vendorAliases = [
        ...
        'FELIPE ANGELO' => ['FELIPE ANGELO PAI ISADORA', 'FELIPE ANGELO (pai Isadora)'],
        'MICROSIS ACORDO' => ['MICROSIS', 'MICROSIS'],
        'THAIS DESPESA WINRAR' => ['THAIS CRISTINA', 'THAIS CRISTINA'],
        'AUTO POSTO JB' => ['AUTO POSTO JB', 'AUTO POSTO JB'],
        'AUTO P JB' => ['AUTO POSTO JB', 'AUTO POSTO JB'],
        'NATIVUS' => ['NATIVUS CARE', 'NATIVUS CARE'],
        'FUSCAMILO' => ['DANIEL CAMILO', 'DANIEL CAMILO'],
        'CIMPECAS' => ['CIMPECAS', 'CIMPEÇAS'],
        'REEMBOLSO' => ['MAYCON IRMAO', 'MAYCON (IRMÃO)'],
        'MARCELO' => ['MARCELO ELETRONICA', 'MARCELO ELETRONICA'],
        'COPROMOTOS' => ['COPROMOTOS', 'COPROMOTOS'],
        'DEPOSITO SAO LUIS' => ['DEP SAO LUIS', 'DEP. SÃO LUIS'],
        'MARA' => ['MARA MAE', 'MARA (MÃE)'],
        'MANINHO' => ['MANINHO MOTO PECAS', 'MANINHO MOTO PECAS'],
        'BRUNO' => ['FABRETTI ADV', 'FABRETTI ADV'],
        'TOP COM INFORMATICA' => ['TOP COM INFORMAT', 'TOP COM INFORMAT.'],
        'PROMOTOS' => ['PROMOTOS', 'PROMOTOS'],
        'JAIR' => ['JAIR SILVEIRA', 'JAIR SILVEIRA'],
        'MATHEUS RISTAU' => ['MATHEUS RISTAU', 'MATHEUS RISTAU'],
        'BRUNAO' => ['BRUNO HENRIQUE PER', 'BRUNO HENRIQUE PER'],
        'WM MOTO PECAS' => ['WM MOTO PECAS', 'WM MOTO PECAS'],
        'MARCENARIA NAVARRO' => ['MARCENARIA NAVARRO', 'MARCENARIA NAVARRO'],
        'TIM CLARO' => ['TIM', 'TIM'],
        'ELETROTRAFO' => ['ELETROTRAFO', 'ELETROTRAFO'],
        'PNEUFREE' => ['PNEUFREE', 'PNEUFREE'],
        'DAISY' => ['DRA DEISY RODRIGUES', 'DRA DEISY RODRIGUES'],
        'COPROPECAS' => ['COPROPECAS', 'COPROPECAS'],
        'MM CARTRUCK' => ['MM CARTRUCK', 'MM CARTRUCK'],
        'AUTO ELETRICA BRASIL' => ['AUTO ELETRICA BRASIL', 'AUTO ELETRICA BRASIL'],
        'CLAUDEMIR DAINEZI' => ['CLAUDEMIR DAINEZI', 'CLAUDEMIR DAINEZI'],
        'VIAGEM' => ['NUBANK', 'NUBANK'],
    ];
...
{
    if (preg_match('/^CARRO\s+\d{1,3}\s+48$/', $normalizedDescription) === 1) {
        return ['status' => 'rule', 'value' => lookupBaseValue($base['categories'], 'FINANC VEICULAR', 'FINANC. VEICULAR'), 'reason' => 'description_rule'];
    }

    if ($normalizedDescription === 'DESPESA') {
        return ['status' => 'rule', 'value' => lookupBaseValue($base['categories'], 'ALIMENTACAO', 'ALIMENTAÇÃO'), 'reason' => 'description_rule'];
    }

    $patternRules = [
        ['/^(ALURA\s+\d{1,2}\s+12)$/', 'EDUCACAO', 'EDUCACAO'],
        ['/^(AR CONDIC\s+\d{1,2}\s+10)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(ESPUMA FONE\s+\d\s+2)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(TANQUINHO CENTRIFUGA\s+\d\s+6)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(FILTRO DE BARRO\s+\d\s+3)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(PURO JEANS\s+\d\s+2)$/', 'VESTUARIO', 'VESTUÁRIO'],
        ['/^(PEDRO LAVACAR\s+\d\s+3)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(CIMPECAS OLEO BLAZER\s+\d\s+2)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(M O MANINHO ENTRADA\s+\d\s+3)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(WAP LAVADORA\s+\d\s+9)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(SERRA LIXADEIRA\s+\d\s+6)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(FOGAO\s+\d{1,2}\s+10)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(SMARTPHONE\s+\d{1,2}\s+10)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(AMAZON NATAL\s+\d\s+2)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(LIQUIDIFICADOR MARA\s+\d\s+4)$/', 'AQUISICAO', 'AQUISIÇÃO'],
        ['/^(RAFAELA LINGERIE KARINA\s+\d\s+2)$/', 'VESTUARIO', 'VESTUÁRIO'],
        ['/^(LEROY MERLIN\s+\d\s+3)$/', 'REFORMA MANUT', 'REFORMA/MANUT.'],
        ['/^(BATERIA BLAZER\s+\d\s+3)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(PIVO BLAZER\s+\d\s+2)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(EXPRESSO AUTO CENTER\s+\d\s+3)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(ACCIOLY REVISAO\s+\d\s+5)$/', 'MANUT VEICULO', 'MANUT. VEICULO'],
        ['/^(COISAS MAITE\s+\d\s+3)$/', 'INFANTIL', 'INFANTIL'],
        ['/^(WISEUP\s+\d{1,2}\s+12)$/', 'EDUCACAO', 'EDUCACAO'],
        ['/^(SUP BOX|SUP BOX\s+\d{2}\s+\d{2}|SUP BOX MES|BOX|BOX\s+\d{2}\s+\d{2}|BOX ATACAD\s+\d{2}\s+\d{2})$/', 'MERCADO', 'MERCADO'],
        ['/^(ALMOCO|ALMOCO FAMILIA\s+\d{2}\s+\d{2})$/', 'ALIMENTACAO', 'ALIMENTAÇÃO'],
        ['/^(C A TINTAS(?:\s+PINTURA CASA)?\s+\d\s+2)$/', 'REFORMA MANUT', 'REFORMA/MANUT.'],
        ['/^(ELETROTRAFO|ELETROTRAFO\s+\d\s+2)$/', 'REFORMA MANUT', 'REFORMA/MANUT.'],
        ['/^(MALUKINHA\s+\d\s+3)$/', 'VESTUARIO', 'VESTUÁRIO'],
    ];

    foreach ($patternRules as [$pattern, $categoryKey, $fallback]) {
        if (preg_match($pattern, $normalizedDescription) === 1) {
            return ['status' => 'rule', 'value' => lookupBaseValue($base['categories'], $categoryKey, $fallback), 'reason' => 'description_rule'];
        }
    }

    $rules = [
        [['SUP CIDADE CANCAO', 'SUP CANCAO', 'SUP CID CANCAO', 'CANCAO', 'BOX ATACADISTA', 'SUP LONDRINA', 'MOLINIS SUP', 'MOLINIS', 'SUP MOLINIS', 'SUP CONDOR', 'VERONA SUP BOULEVARD', 'CASA DE CARNE VALENTE', 'ACOUGUE CARNE', 'ACOUGUE SAO JOSE', 'ATAK MERCEARIA', 'EMELINE AROMAS', 'LUVA DESC TALGE', 'CERVEJA NILDO CAMILO', 'PANIF PAO PURO', 'AMERICANAS', 'REFRIGERANTE CONQUISTA'], 'MERCADO', 'MERCADO'],
        [['SUP CENTER', 'SUPERMERCADO', 'CARREFOUR', 'MAXI LIMPEZA', 'DISTRIBUIDORA PRIMAVERA', 'DISTRIB PRIMAV', 'ECOBRILHO'], 'MERCADO', 'MERCADO'],
        [['SPOTIFY', 'YOUTUBE'], 'STREAMING', 'STREAMING'],
        [['PADOKA', 'IFOOD', 'LANCHE', 'LANCHONETE', 'ALMOCO CATUAI', 'VADECO', 'SANTA PIZZA', 'COSTELA GRILL', 'DOG KING', 'PARRA S BURGER', 'CASA DE CARNE PADROEIRA', 'DIVINO FOGAO', 'ESQUINA DO ESPETO', 'OUTBACK', 'PALITOS BURGER', 'PASTEL JORGE FEIRA', 'REST BARTOLO LONDRINA', 'FEIJOADA PAI DO BRUNO', 'FRANGO ASSADO CLEBER', 'KITUTS', 'TRAILER BOTAFOGO', 'MC DONALDS DRIVE THRU', 'PIZZARIA DOM PARMELO', 'DOM PARMELO', 'DOM PARMELLO', 'BUONN FRATELLO PIZZARIA', 'BUONN FRATELLO', 'GELA BOCA', 'DELLA PAZZETTI', 'DELLA PAZZETI', 'PIZZARIA DELLA NONA', 'PIZZARIA DELLA NONNA', 'DELLA NONA', 'PADARIA JOIA', 'PADARIA', 'JOIA', 'RESTAURANTE', 'BURGER KING', 'BUENOS BURGER', 'DI BURGER', 'CHAPAO BURGER', 'COSTELA', 'COSTELLA', 'VILA ITALIA', 'REST COSTELA', 'STRASSBERG', 'HANGAR', 'PIZZARIA VILA ITALIA', 'BUENO BURGER', 'TODO TORTO', 'REST DOM JOAQUIM', 'CROASSONHO', 'CROASONHO', 'PIZZARIA FORNINHO', 'LA MAFIA HAMBURG', 'LOS BRAGAS', 'PIZZARIA CAPRIOLLI', 'PIZZARIA NOVA PACHECO', 'PIZZA POPEDI', 'PIZZA', 'PEDACOS DE AMOR', 'GELOBEL LANC DON PABLO', 'GELOBEL', 'SUBWAY', 'PIZZARIA LAS VEGAS', 'KOJO', 'BEBIDAS', 'CHIQUINHO', 'TROPICANA', 'ESTACAO PROCOPIO', 'HACHIMITSU', 'FOOD TRUCK', 'SORVETERIA KI DELICIA', 'BENDROLL', 'BEND ROLL', 'BEND E ROLL', 'PAO E VINHO FRANGO ASSADO', 'CHULETA MALUCA STEAKBAR', 'CAFE DO PARQUE', 'PETISCARIA DIMIRSU', 'RICKS STREET FOOD INBOX', 'RICKS BURGUER', 'REST TRADI MINEIRA', 'REST TRADICAO MINEIRA', 'RANCHO DOS PAMPAS REST', 'MC DONALDS SORVETE', 'DISTRIBUIDORA MARCAO CERV KARINA', 'TOGETHER', 'DOCE HISTORIA CAFE', 'ACAI WAVE', 'ACAI', 'BARILOCHE ALMOCO', 'PONTO GRILL BOULEVARD'], 'ALIMENTACAO', 'ALIMENTAÇÃO'],
        [['AUTO POSTO', 'AUTO P JB', 'GASOLINA', 'ETANOL', 'ALCOOL', 'COMBUSTIVEL', 'PALOMA', 'POSTO IPIRANGA CP', 'POSTO JB', 'POSTO SHANGRI LA'], 'COMBUSTIVEL', 'COMBUSTÍVEL'],
        ...
        [['PETSHOP CANTINHOS', 'LOJAO RURAL', 'KEILA PET BETOVEN', 'PETZ'], 'PET', 'PET'],
        [['PETITE BOX', 'LEITE MAITE', 'NISSEI MAITE', 'FRALDA PAMPERS', 'BABA MAITE', 'MALUKINHA KIDS', 'PRESENTE MAITE', 'ESTILIO PAVIANI MAITE'], 'INFANTIL', 'INFANTIL'],
        [['FARMACIA', 'DENTISTA', 'SAUDE', 'REMEDIO', 'MEDICO MAITE', 'FARMACINHA DA MAITE', 'AMAZON BICO CHUPETA', 'CECOR EXAME', 'USIFARMA VERONA', 'GFARMA AVENIDA', 'FRALDA BAYGON KARINA', 'FRALDA MAITE', 'DROGA RAIA', 'GRANADO', 'ACADEMIA', 'KIT MANICURE ISA', 'JOAO LIMA INJECAO ISADORA', 'LUCIA PODOLOGA', 'STARCLIN', 'SANTA CASA EXAME COVID'], 'SAUDE', 'SAÚDE'],
        [['NISSEI'], 'SAUDE', 'SAÚDE'],
        [['CREDITO CELULAR', 'PLANO TIM', 'PLANO CLARO', 'CLARO DIAS', 'MAYCON TIM'], 'TELEFONIA', 'TELEFONIA'],
        [['CELULAR MAURICIO'], 'TELEFONIA', 'TELEFONIA'],
        [['INTERNET BRASILNET', 'INTERNET VISAONET', 'BRASILNET INSTALACAO'], 'INTERNET', 'INTERNET'],
        [['IPVA', 'DOC MOTO', 'DOCUMENTO VIRAGO', 'MULTA HEHE', 'LICENCIAMENTO'], 'IPVA', 'IPVA'],
        [['POS GRADUACAO', 'UNIFIL', 'FACULDADE', 'INGLES'], 'APRENDIZADO', 'APRENDIZADO'],
        [['BUSAO', 'NOVA MARIVAL', 'CERTIF PITAGORAS', 'APOSTILA ST MICROCOPY', 'MAT ESCOLAR MARIVAL', 'MAT ESCOLAR ISADORA', 'CADERNO ISADORA', 'LIVRARIAS CURITIBA', 'CURSO UDEMY', 'WISEUP'], 'EDUCACAO', 'EDUCACAO'],
        [['FIES'], 'FINANC ESTUDANTIL', 'FINANC. ESTUDANTIL'],
        [['PREVIDENCIA', 'APOSENT'], 'APOSENTADORIA', 'APOSENTADORIA'],
        [['AGUA'], 'AGUA', 'AGUA'],
        [['ENERGIA'], 'ENERGIA', 'ENERGIA'],
        [['ALUGUEL', 'AUXILIO EM CASA', 'RESTANTE FEVEREIRO MARCIO PAI'], 'ALUGUEL', 'ALUGUEL'],
        [['CARRO 43', 'CARRO 44', 'CARRO 45', 'CARRO 46', 'CARRO 47', 'CARRO 48'], 'FINANC VEICULAR', 'FINANC. VEICULAR'],
        [['RIZZO', 'ESTAC SHOP BOULEVARD'], 'ESTACIONAMENTO', 'ESTACIONAMENTO'],
        ...
        [['MAKITA', 'CAPACETE MERCADO LIVRE', 'SAO JOAO CARTAO UMIDIF', 'MERCADO LIVRE MALETA', 'MERCADOLIVRE', 'LUVA MERC LIVRE', 'SHOPEE', 'A BARATEIRA', 'HAVAN', 'PULSEIRA RELOGIO', 'KIT CELULAR KARINA', 'REPARO CELULAR KARINA', 'TRAVESSEIRO ORTOBOM', 'PEDALON LIQUIDIFICADOR', 'ITENS COZINHA ML', 'LOJAS TUDO 10 COISA P COZ', 'POLTRONA MARCIO', 'MANINHO CAPACETE', 'JOGO DE FACA', 'JOGO DE PANELA', 'FURADEIRA', 'KIT PARAFUSADEIRA', 'GAS BUTIJAO RESERVA', 'CLIMATIZADOR', 'AR CONDIC', 'WAP LAVADORA', 'SERRA LIXADEIRA', 'FOGAO', 'SMARTPHONE', 'AMAZON NATAL', 'LIQUIDIFICADOR MARA', 'ESPUMA FONE', 'TANQUINHO CENTRIFUGA', 'FILTRO DE BARRO', 'PRESENTE FADONI', 'PRESENTE ROSE REGINALDO', 'MATHEUS RISTAU BINGO', 'ALIANCA NAMORO', 'COLCHAO INFLAVEL', 'CAPA PALETA', 'SOFA', 'CABECEIRA', 'KIT JARDINAGEM MARA', 'KIT CROCHET MARA', 'PRESENTE MARA', 'PRESENTE ISADORA', 'PRESENTE MAE DA ISA', 'PRESENTE ROGER', 'PRESENTE KARINA', 'MOCHILA ISADORA', 'KIT AGULHA MARA', 'SERRA COPO WD40', 'KIT POLIMENTO', 'KIT CHAVE CATRACA', 'FERRAMENTAS ELETROTRAFO', 'PRESENTE TAI', 'AMORT CADEIRA UNIMAQ', 'AMIGO SECRETO E PERFUME', 'PRESENTE AMIGO SECRETO', 'ELETROBARROS RODIZIO SOFA', 'LUMINARIA MESA', 'ACM SUPORTE ESPELHO', 'TRENA A LASER PAULO', 'SMARTPHONE KARINA', 'SOCOPIAS PADRINHOS', 'KIT BROCA CHINA', 'BANDOLEIRA', 'LANTERNA TATICA', 'RADIO BAOFENG', 'RELOGIO CHINA', 'FLORES SHOPPING', 'CAPACETE 1 3', 'CAPACETE 2 3', 'CAPACETE 3 3', 'ALGO DO MAYCON', 'MATERIAIS ELETRICOS', 'PGTO CHICOTE MAURICIO', 'TV LG SMART 49 LED 4K', 'PLIMOR', 'CAMISAS MAYCON', 'SOM QUARTO', 'CABO SOM'], 'AQUISICAO', 'AQUISIÇÃO'],
        [['GAS'], 'GAS', 'GÁS'],
        [['TELHANORTE', 'DEP SAO LUIS', 'DEP SAO LUIZ', 'BOX BANHEIRO', 'REFORMA CASA', 'MATERIAL DE CONSTRUCAO', 'CATIVA TELA MOSQUITEIRA', 'ELETROTRAFO RESISTENCIA CHUV', 'ELETROTRAFO REPARO BANHEIRO', 'ELETROBARROS RESISTENCIA', 'JH MAT CONSTRUCAO BAN MAITE', 'MANG CHUVEIRO SUITE', 'NEGRIPAR PARAFUSO SALA', 'SUPORTE REDE', 'LEROY MERLIN', 'COLOR TINTAS', 'MATERIAL ELETRICO', 'MAT ELETRICO KARINA', 'INSTALACAO AR CONDICIONADO'], 'REFORMA MANUT', 'REFORMA/MANUT.'],
        [['MICROSIS', 'THAIS CRISTINA DE LIMA', 'ADVOGADO BRUNO', 'BRUNO FABRETTI ADVOGADO', 'PREFEITURA MUNICIP', 'REPASSE A MAYCON', 'PARTE MAYCON'], 'DESP EMPRESA', 'DESP. EMPRESA'],
        [['DEPOSITO BANCO', 'DEPOSITO BANCO CONJU', 'NUBANK RENDIMENTO', 'GUARDADO NUBANK'], 'INVESTIMENTO', 'INVESTIMENTO'],
        [['GETNET'], 'IMPOSTO EMPRESA', 'IMPOSTO (EMPRESA)'],
        [['ANUIDADE CARTAO', 'ANUIDADE', 'TARIFA MERCADO PAGO'], 'INVESTIMENTO', 'INVESTIMENTO'],
        [['CARTAO MARCIO'], 'EMPRESTIMO', 'EMPRÉSTIMO'],
        [['GOL QUADRADO', 'MOTO FEVEREIRO', 'REPARO MOTO MOLA', 'MOTOR VIRAGO', 'DOCUMENTO VIRAGO', 'BATERIA VIRAGO', 'PAINEL MOTO', 'POSTO OLEO DA MOTO', 'PECAS VIRAGO', 'CENTER CAR', 'MOTO JANEIRO FEV', 'CORRENTE MOTO', 'RETIFICADOR VIRAGO', 'ADITIVO BLAZER', 'ROLAMENTO VIRAGO', 'OLEO MILITEC VIRAGO', 'FLUIDO FREIO DOT 4 VIRAGO', 'EXPRESSO AUTO CENTER', 'PONTEIRA ESCAPE BLAZER', 'PIVO BLAZER', 'BATERIA BLAZER', 'CIMPECAS OLEO BLAZER', 'ACCIOLY REVISAO', 'DALLAS TROCA DE OLEO BLAZER', 'JAIR PINTURA ML E RODA', 'MANUTENCAO MOTO', 'BIELETA SUSPENSAO', 'PALHETA GOL QUADRADO', 'OFICINA MECANICA REVISAO', 'AUTO ELETRICA REPARO LUZ TETO', '4 PNEUS BLAZER', 'RETENTOR DIFERENCIAL', 'OLEO CAMBIO 80W90 LUBRAX', 'METADE OLEO GOL QUADRAD', 'MM CAR TRUCK PEUGEOT', 'RELE VIRAGO', 'VOLANTE SAIDA DE AR COIFA', 'SUFILM GOL', 'ALARME CENTRALINA SUFILM', 'CONNECT GOL', 'PARACHOQUE GOL', 'KIT BOTAO GOL', 'CALOTA GOL', 'BORRACHA DO GOL KAYABA', 'PLACA DE PARTIDA VIRAGO', 'TUTI CARBURADOR', 'EMBREAGENS COPROCAR', 'ITIBAN VER 60K', 'WM MOTOPECAS', 'MANUTENCAO GOL EMBRAGENS COPRO', 'AUTO ELET BRASIL VIRAGO', 'MOLA ESPORTIVA', 'PECAS BLAZER', 'PECAS BLAZER GM', 'OLEO VIRAGO', 'FILTRO DE OLEO VIRAGO', 'MANOPLA COIFA COCKPIT', 'ENGRENAGEM BOMBA DE OLEO', 'SENSOR DE OLEO VIRAGO', 'JOGO DE JUNTA VIRAGO', 'KIT BIELA VIRAGO', 'KIT PISTAO BIZ 100 ANEL', 'CANETINHA PNEU', 'MANINHO CARBURADOR', 'COLETOR VIRAGO', 'MOTO MAYCON', 'CDI VIRAGO', 'CABO AC REPARO PROMOTOS', 'BOBINA VIRAGO', 'CABO AC VIRAGO', 'FILTRO ESPORTIVO VIR', 'RACE CUSTOM VIRAGO', 'RACECUSTOM', 'GRELHA VIRAGO', 'RELACAO VIRAGO WM MOTO', 'FAROL FREIO VIRAGO', 'EIXO QUADRO VIRAGO', 'KIT RAIO VIRAGO', 'KIT SETA VIRAGO', 'ELETRICA VIRAGO', 'PNEU VIRAGO', 'CABO DO FREIO TRAS', 'PNEU CG 150', 'MANINHO REF VIRAGO', 'CBR 450SR MANINHO', 'ALI EXPRESS ADAPTADOR RETROVISOR', 'KIT ADESIVO ML', 'KIT LATERAL ML', 'KIT EMBLEMA ML'], 'MANUT VEICULO', 'MANUT. VEICULO'],
        [['MARA MESADA', 'BIPE SNIPER', 'STEAM DLC ETS2', 'DLC ETS2', 'DLC ETS', 'EURO TRUCK DLC PAINTS', 'ACE COMBAT 7', 'LEFT 4 DEAD', 'EPIC', 'RETORNAR', 'ONG ORAR F75', 'ONG RETORNAR HD 883', 'PRISTON TALE', 'EURO TRUCK COMPLETO', 'EURO TRUCK GOING EAST', 'JOGO CITIES SKYLINES', 'VOLANTE SCANIA', 'LOGITECH G27', 'JOYSTICK AVIAO', 'LUNETA SNIPER', 'KIT MILITAR', 'KIT CAPACETE AIRSOFT', 'FARDA AIRSOFT', 'GTA V'], 'INVEST MAURICIO', 'INVEST. MAURICIO'],
        [['TNS CABOS', 'TNS FONTE PC SONECA', 'FONTE 500W EVA', 'FONTE KARINA TECL BRUNO', 'MONITOR FONTE', 'TECLADO ALL IN ONE', 'CABO DE REDE CAT6 ADM INF', 'CAPACITOR ELETROLITICO', 'FLAT NOTEBOOK FABRETTI', 'FLAT NOTEBOOK DELL', 'CHIP IPHONE GVEI', 'TECLADO NOTE GUI SEMP', 'TECLADO NOTE KAREN', 'REGISTROBR', 'ONEDRIVE', 'MODEM ROTEADOR 3G', 'ACCESS POINT INTELBRAS', 'BATERIA NOBREAK', 'FONE AKG', 'HOSPEDAGEM MAYCON', 'MOCHILA ACER', 'PLACA DE VIDEO', 'MONITOR 30', 'INVERTER NOTEBOOK DELL FERNANDA', 'ANTENA EXTERNA', 'HD SSD', 'SSD'], 'TECNOLOGIA', 'TECNOLOGIA'],
        [['CAMISETA PITICAS', 'PERNAMBUCANAS', 'RAFAELLA LINGERIE', 'RAFAELA LINGERIE KARINA', 'KIT CUECA', 'KIT COTURNO', 'TENIS CINTO MEIA', 'UNIFORME MARISA MODAS', 'LUVA MAURICIO', 'LUVA MOTO', 'KIT TENIS', 'KIT CAMISA', 'KIT SHORT', 'TENIS', 'MEIA LUPO', 'CALCA MOLETOM', 'MOLETOM FUEL TECH', 'BLUSA KARINA', 'JALECO KARINA', 'TUDO10 BIKINI LINGUI PORTA', 'LENCOL CAMA'], 'VESTUARIO', 'VESTUÁRIO'],
        [['RIACHUELO'], 'VESTUARIO', 'VESTUÁRIO'],
        [['LUZ CAMILA TESTE', 'FUNWORK TESTE', 'TESTE BUZZ', 'DOMINIO CEDIMAGEM', 'TESTE EMPORIOLUZ', 'CONTROLLE BUZZ', 'ELETROBARROS BUZZ', 'ELTROBARROS BUZZ', 'A TRICOLANDIA BUZZ', 'PORTA DE PAPEL BUZZ', 'ITENS ELETRICOS BUZZ', 'TOK PLENO TESTE BUZZ'], 'BUZZ', 'BUZZ'],
    ];
